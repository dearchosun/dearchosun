...
<?php require_once('./classes/xml/XmlJsFilter.class.php');$__xmlFilter=new XmlJsFilter('modules/member/skins/sketchbook5_member_skin/filter','openid_login.xml');$__xmlFilter->compile(); ?>
<div class="signin">
	<div class="login-header">
		<h1 class="h1"><i class="icon-user"></i> <?php echo $__Context->lang->cmd_login ?></h1>
	</div>
	<div class="login-body">
		...
		<?php Context::addJsFile("./files/ruleset/login.xml", FALSE, "", 0, "body", TRUE, "") ?><form  action="<?php echo getUrl('') ?>" method="post" id="fo_member_login"><input type="hidden" name="error_return_url" value="<?php echo htmlspecialchars(getRequestUriByServerEnviroment(), ENT_COMPAT | ENT_HTML401, 'UTF-8', false) ?>" /><input type="hidden" name="mid" value="<?php echo $__Context->mid ?>" /><input type="hidden" name="vid" value="<?php echo $__Context->vid ?>" /><input type="hidden" name="ruleset" value="@login" />
			<input type="hidden" name="success_return_url" value="<?php echo $__Context->referer_url ?>" />
			<input type="hidden" name="act" value="procMemberLogin" />
			<input type="hidden" name="xe_validator_id" value="modules/member/skins" />
			<fieldset>
				<div class="control-group">
					<?php if($__Context->identifier == 'user_id'){ ?><input type="text" name="user_id" id="uid" required placeholder="<?php echo $__Context->lang->user_id ?>" title="<?php echo $__Context->lang->user_id ?>" autocapitalize="off" style="ime-mode:inactive;" /><?php } ?>
					<?php if($__Context->identifier != 'user_id'){ ?><input type="email" name="user_id" id="uid" required placeholder="<?php echo $__Context->lang->email_address ?>" title="<?php echo $__Context->lang->email_address ?>" autocapitalize="off" style="ime-mode:inactive;" /><?php } ?>
				...
				<div class="control-group">
					...
					<input type="submit" value="<?php echo $__Context->lang->cmd_login ?>" class="submit btn" />
				</div>
			</fieldset>
		</form>
	</div>
