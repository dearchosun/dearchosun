...
echo $__Context->lang->user_id;
					...
					<div class="checkbox">
						<label for="keep_signed" onclick="jQuery('#modal-login input[name=\'keep_signed\']').click();"><input type="checkbox" name="keep_signed" value="Y" onclick="if(this.checked) return confirm('<?php echo $__Context->lang->about_keep_signed ?>');" /> <?php echo $__Context->lang->keep_signed ?></label>
					</div>
...
if($__Context->key!= $__Context->lang_type){ ?>
			...
			<?php if($__Context->li->premium != 'Y'){ ?><li><a href="http://www.wincomi.com" target="_blank"><i class="fa fa-check" area-hidden="true"></i> Layout by Wincomi</a></li><?php } ?>
		</ul>
	</div>
